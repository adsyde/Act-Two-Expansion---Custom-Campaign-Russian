...
| Строк в моде | 4987 |
| Переведено | **4774 (95.7%)** |
| Не переводится | 213 — 65 записей с пустым оригиналом и 148 технических идентификаторов |

Переведён весь текст мода, включая служебные подписи редактора. Английскими
остались только две категории, где перевод невозможен или запрещён правилами
проекта: 65 записей, у которых оригинал пуст, и 148 технических
идентификаторов вида `AiHint_Mage`, `IGNORE_HEIGHT_DIFFERENCE`,
`CHI_ARM_Cloth_Test4` — это имена флагов и ресурсов, а не текст.
...
Работает и с обычной сборкой мода, и с вариантом **QuickStart**: у них один
UUID и одинаковые идентификаторы строк, различаются только четыре номера версий.
...
— это сортировка для показа, а не порядок загрузки. Привязки к конкретной сборке оригинала нет: обновление
